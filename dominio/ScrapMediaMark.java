...
public class ScrapMediaMark extends Agent{
	...
	public class Manejador extends AchieveREResponder {
		...
		protected ACLMessage prepareResultNotification(ACLMessage request, ACLMessage response) throws FailureException{
			List<Movil> todos = scrapMediaMark(request.getContent());
			if(todos.size()==0){
	...
	public List<Movil> scrapMediaMark(String nombre_movil) {
		String baseUrl = "https://www.mediamarkt.es/es/search.html?query=";
		@SuppressWarnings("deprecation")
		String urlPage = baseUrl + URLEncoder.encode(nombre_movil) + "&searchProfile=onlineshop&channel=mmeses";
		...
		if (ScrapFunciones.getStatusConnectionCode(urlPage) == 200) {
			...
			Elements entradas = document.select("div.product-wrapper");

			Pattern patron = Pattern.compile("Móvil.*");

			for (Element elem : entradas) {

				String precio_string = elem.select("div.price-box").select("div.price.small").text();

				if (!precio_string.isEmpty() && precio_string != null) {
