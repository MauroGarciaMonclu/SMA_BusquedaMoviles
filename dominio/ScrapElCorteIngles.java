...
public class ScrapElCorteIngles extends Agent{
	...
	public class Manejador extends AchieveREResponder {
		...
		protected ACLMessage prepareResultNotification(ACLMessage request, ACLMessage response) throws FailureException{
			List<Movil> todos = scrapElCorteIngles(request.getContent());
			if(todos.size()==0){
	...
	public List<Movil> scrapElCorteIngles(String nombre_movil) {
		String baseUrl = "https://www.elcorteingles.es/electronica/moviles-y-smartphones/search/?level=6&s=";
		...
		if (ScrapFunciones.getStatusConnectionCode(urlPage) == 200) {
			...
			Elements entradas = document.select("div.product-preview");

			Pattern patron = Pattern.compile("(.*Funda.*)|(.*Protection.*)|(.*Silicona.*)|(.*Carcasa.*)|(.*Protector.*)|(.*Cristal.*)|(.*Carcas.*)|(.*Pulsera.*)");

			for (Element elem : entradas) {

				String precio_string = elem.select("div.product-price").select("span.current").text().replaceAll("\u20ac", "");
				...
				if ((precio_string != null) && (!precio_string.isEmpty())) {
					...
					double precio = Double.parseDouble(precio_string.replace(",", "."));
					String nombre = elem.select("div.product-name").select("a.js-product-click").text();
					...
					if (!igual.matches()) {
						...
						if (ScrapFunciones.AnadirMovil(nombre, nombre_movil)) {
							Movil movil = new Movil(nombre, precio, "El Corte Ingles");
